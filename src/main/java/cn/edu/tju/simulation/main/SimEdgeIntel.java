 ...
 * 
 */
public class SimEdgeIntel extends JFrame {
	 ...
	 * Log
	 */
	private static Logger logger = Logger.getLogger(SimEdgeIntel.class);
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SimEdgeIntel window = new SimEdgeIntel();
	...
	/**
	 * Add a context to the emulator and initialize the main window. In
	 * addition, the main window adds listeners.
	 */
	private void initial() {
		{
			// Instantiate all components and context
			Context context = new Context();
			this.map = new Map();
			this.operator = new Operator();
			this.log = new Log();
			this.menu = new MenuBar();
			context.initialController(menu, map, operator, log, context);
		}

		{
			...
			mainFrame.setTitle("SimEdgeIntel");
		...
		public void windowClosing(WindowEvent e) {
			if (Signal.SAVE == false) {
				int choice = JOptionPane.showConfirmDialog(null,
						"The paramaters has not been saved, whether to quit?",
