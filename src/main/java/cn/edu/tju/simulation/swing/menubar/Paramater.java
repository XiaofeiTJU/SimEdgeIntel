...
public class Paramater extends JFrame{
	...
	/**
	 * Text Field£º maximum value of devices's popularity fluctuation interval
	 */
	...
	public Paramater(Container frame, Context context){
	...
	public void initial(){
		...
		
		{
			...
			this.timeSlicesNumber_Text.setText(context.getOperationPanel().getTimeSlices().getSelectedItem().toString());
	...
	public void coverParamater(){
		...
		context.getOperationPanel().getTimeSlices().setSelectedItem(Integer.parseInt(this.timeSlicesNumber_Text.getText()));
		
		ContentService.initialPopularity(context.getOriginalContentList());
	}
